...
struct RComponent : RHandle<struct RComponentObj>
{
    ...
    void add_private_image(Name name, RFormat format, uint32_t width, uint32_t height, RSamplerInfo* sampler = nullptr);

    /// @brief declare an image that can be used by another component as input
    void add_output_image(Name name, RFormat format, uint32_t width, uint32_t height, RSamplerInfo* sampler = nullptr);

    /// @brief declare an image that references some output image of another component
    void add_input_image(Name name, RFormat format, uint32_t width, uint32_t height);

    /// @brief declare an image that references some output image of another component,
    ///        and can be used by another component as input.
    void add_io_image(Name name, RFormat format, uint32_t width, uint32_t height);
...
struct RGraph : RHandle<struct RGraphObj>
{
    ...
    RComponent add_component(Name name);

    /// @brief connects an output image of a Component to some input image of another Component
    void connect_image(Name srcComp, Name srcOutImage, Name dstComp, Name dstInImage);
    ...
    void connect_swapchain_image(Name srcComp, Name srcOutImage);
