...
{
    RectSplit<TestNode> partition(Rect(0.0f, 0.0f, 100.0f, 100.0f));
    ...
    uint32_t node2 = partition.split_right(node1, 0.25f, 0.0f);
    CHECK(node2);

    uint32_t node3 = partition.split_bottom(node2, 0.25f, 0.0f);
    CHECK(node3);

    partition.visit(partition.get_root_id(), [&](TestNode* leaf) {
        if (leaf->nodeID == node1)
            CHECK(leaf->area == area1);
        else if (leaf->nodeID == node2)
            CHECK(leaf->area == area2);
        else if (leaf->nodeID == node3)
            CHECK(leaf->area == area3);
    });
}
...
TEST_CASE("RectSplit top left")
{
    ...
    uint32_t node2 = partition.split_top(node1, 0.25f, 0.0f);
    CHECK(node2);

    uint32_t node3 = partition.split_left(node2, 0.25f, 0.0f);
    CHECK(node3);
