...
#include <LudensBuilder/AssetBuilder/AssetSource/AudioAssetSource.h>
...
void audio_clip_asset_import(void* user)
{
    ...
    std::string sourcePath = info.srcFile.string();
...
bool audio_clip_asset_import_info_set_src_files(AssetImportInfo* base, size_t count, const FS::Path* srcFilePaths)
{
    if (count != 1)
        return false;

    auto* info = (AudioClipAssetImportInfo*)base;
